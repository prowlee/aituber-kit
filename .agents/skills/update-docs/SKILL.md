...
### Step 8: スクリーンショットの確認・追加

コミット前に、新規追加・変更されたUI要素に対してスクリーンショットが必要かどうかを確認し、必要ならCodexが撮影・配置・ドキュメントへの参照追加まで行う。
...
3. 必要と判断したスクリーンショットは、ローカルアプリを起動してPlaywright等で撮影する
4. 生成画像を目視確認し、必要なら撮り直す
5. 日本語・英語・中国語の対応ページに同じ画像を追加する

#### スクリーンショットの配置ルール

- 画像は `./aituber-kit-docs/public/images/` に配置する
- ファイル名は `{カテゴリ}_{ランダム5文字}.webp` を基本とする（例: `vrm_pose_a3b2c.webp`）
- 画像は3言語で共有するため、UI上のテキストが特定言語に依存しない状態が望ましい（日本語UIでも可）
- ドキュメント内に `![説明文](/images/ファイル名)` を追記する
...
ユーザーへの撮影依頼は、ログイン・CAPTCHA・実機操作などCodexだけでは撮影状態を作れない場合に限る。その場合も、必要な画像、配置先、想定ファイル名、撮影すべき画面状態を具体的に伝える。
